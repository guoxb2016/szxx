...
public class SanitationSummaryController extends JeecgController<SanitationSummary, ISanitationSummaryService> {
	...
	public void exportXls3(HttpServletRequest request, SanitationSummary sanitationSummary, HttpServletResponse response) {
		...
		List<SanitationSummary> list = sanitationSummaryService.findGroupByOrg(sanitationSummary.getSysOrgName());
		InputStream in;
		try {
			in = new FileInputStream(new File(template));
			HSSFWorkbook book = null;
			...
			//根据type wb.removeSheetAt(wb.getSheetIndex(sheetName)); 
			String scope = "";
			if(sanitationSummary.getScope() != null) {
				scope = sanitationSummary.getScope();
			}
			switch (scope) {
				case "环卫职工":
					book.removeSheetAt(book.getSheetIndex("环卫车辆")); 
					book.removeSheetAt(book.getSheetIndex("环卫设施")); 
					book.removeSheetAt(book.getSheetIndex("道路清扫保洁")); 
					book.removeSheetAt(book.getSheetIndex("环卫工人福利")); 
					book.removeSheetAt(book.getSheetIndex("环卫市场化情况")); 
					break;
				case "环卫车辆":
					book.removeSheetAt(book.getSheetIndex("环卫一线工人")); 
					book.removeSheetAt(book.getSheetIndex("环卫设施")); 
					book.removeSheetAt(book.getSheetIndex("道路清扫保洁")); 
					book.removeSheetAt(book.getSheetIndex("环卫工人福利")); 
					book.removeSheetAt(book.getSheetIndex("环卫市场化情况"));
